...
export const UserManagement: React.FC = () => {
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const expectedHeader = "id,name,email,phone,city,is_active";
      const fileHeader = lines[0].trim().toLowerCase();

      if (fileHeader !== expectedHeader) {
        throw new Error(
          "Invalid CSV format. Expected columns: id,name,email,phone,city,is_active"
        );
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        usersArray.push({
          // Skip id (values[0]) - will be auto-generated by backend
          name: values[1],
          email: values[2],
          phone: values[3],
          city: values[4],
